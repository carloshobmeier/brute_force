...
def brute_force_decrypt(xlsx_path, max_length, character_set):
    file = msoffcrypto.OfficeFile(open(xlsx_path, "rb"))
...
if __name__ == "__main__":
    xlsx_path = input("Digite o caminho do arquivo XLSX criptografado: ")
    ...
    if character_set:
        result = brute_force_decrypt(xlsx_path, max_length, character_set)
        if result:
            print(f"A senha do XLSX é: {result}")
        else:
            print("Não foi possível descriptografar o XLSX com as combinações testadas.")
    else:
